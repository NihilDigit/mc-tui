//! `shulker new <dir>` subcommand: scaffold a fresh Paper/Purpur server.
//!
...
#[derive(Parser, Debug)]
#[command(name = "shulker", about, version)]
pub struct Cli {
    /// Path to the Minecraft server directory (must contain server.properties).
    /// If omitted, falls back to the value remembered in $XDG_CONFIG_HOME/shulker/state.toml.
...
// ---------- v0.4: server scaffolder (`shulker new <dir>`) ----------
...
pub fn scaffold_new(
    ...
    // 5. eula.txt — only the user can ethically agree, but `shulker new` is interactive intent.
    //    We write `eula=true` since the user invoked us with explicit intent to run a server.
    fs::write(dir.join("eula.txt"), "# generated by shulker new\neula=true\n")
        .context("write eula.txt")?;
    eprintln!("✓ wrote eula.txt");

    // 6. Per-OS start scripts with Aikar's flags + heap based on RAM.
    // We always write start.sh (POSIX hosts run shulker via that path) AND on
    // Windows hosts we additionally write start.bat — shulker's `start_server`
    // picks the OS-native one. Authoring the .bat from a Linux host is fine
    // too (lets you scaffold a server on one box, ship the dir to another).
    let heap_mb = recommended_heap_mb();
    let start_sh = aikar_start_script_sh(&jar_name, heap_mb);
    let start_path_sh = dir.join("start.sh");
    fs::write(&start_path_sh, start_sh).context("write start.sh")?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let mut perms = fs::metadata(&start_path_sh)?.permissions();
        perms.set_mode(0o755);
        fs::set_permissions(&start_path_sh, perms)?;
    }
    eprintln!("✓ wrote start.sh (heap: {}M)", heap_mb);

    let start_bat = aikar_start_script_bat(&jar_name, heap_mb);
    let start_path_bat = dir.join("start.bat");
    fs::write(&start_path_bat, start_bat).context("write start.bat")?;
    eprintln!("✓ wrote start.bat (heap: {}M)", heap_mb);
    ...
    eprintln!("  shulker --server-dir {}", dir.display());
    Ok(())
}
...
fn aikar_start_script_bat(jar: &str, heap_mb: u32) -> String {
    // CRLF line endings — required so cmd.exe parses each line correctly when
    // the file is dropped onto a Windows host. Aikar's flags translated to
    // single-line form; cmd's `^` line continuations work but are fragile, so
    // we ship one big line.
    format!(
        "@echo off\r\n\
         REM Generated by shulker new. Edit freely.\r\n\
         cd /d \"%~dp0\"\r\n\
         set JAR={jar}\r\n\
         set HEAP={heap_mb}M\r\n\
...
Java(TM) SE Runtime Environment ..."#;
    ...
    #[test]
    fn aikar_sh_script_contains_essentials() {
        let s = aikar_start_script_sh("paper.jar", 4096);
    ...
    #[test]
    fn aikar_bat_script_contains_essentials() {
        let s = aikar_start_script_bat("paper.jar", 4096);
        assert!(s.contains("paper.jar"));
        assert!(s.contains("4096M"));
        assert!(s.contains("UseG1GC"));
        assert!(s.contains("--nogui"));
        ...
        // CRLF — required so cmd.exe parses the file when it lands on Windows.
        assert!(s.contains("\r\n"));
    }
